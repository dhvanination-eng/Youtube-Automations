# Survival Nature Setup Instructions
...
```bash
python batch_generate.py channel_survival --count 100
```
...
- **Local Render Only**:
  ```bash
  python run_channel.py channel_survival
  ```
- **Render and Auto-Upload**:
  ```bash
  python run_channel.py channel_survival --upload
  ```
